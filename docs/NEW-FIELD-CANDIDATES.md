...
| **Wet/dry blend `w`** ⭐ | Data 0xD5D6C | 0 (dry) | **Statically patchable — no GP2Lap needed.** Q14 lerp @0x1FB67: `coef = (max*(0x4000-w) + min*w) >> 14` across all three surface tables. `w=0x4000` = fully wet. **No writer anywhere in the binary**, so an EXE patch persists; GP2Lap is only needed to *vary* `w` (drying line, rain mid-race). Player-only in practice — AI cars follow precomputed dry pace tables and won't slow down. **Caveat:** enabling this makes the `min` tables live, and `min[0]` of each is aliased as class 5 (kerb back-apron) via an OOB read — see `weather-remnants.md`. Not in the current plan; needs its own design. |
