...
use windows_sys::Win32::{
	Foundation::{GetLastError, GlobalFree, HANDLE, HGLOBAL, POINT},
...
fn to_final_path_wide(p: &Path) -> Option<Vec<u16>> {
	...
	fill_utf16_buf(
		...
		|buf| {
			// Strip the `\\?\` / `\\?\UNC\` namespace prefix that
			// GetFinalPathNameByHandleW returns. This mirrors PathCchStripPrefix
			// in pure Rust so we don't import it from api-ms-win-core-path-l1-1-0.dll,
			// a Win8+ API set that is absent on Windows 7 (GP2-Workshop targets Win7).
			const NS: &[u16] = &[b'\\' as u16, b'\\' as u16, b'?' as u16, b'\\' as u16];
			const UNC: &[u16] = &[
				b'\\' as u16, b'\\' as u16, b'?' as u16, b'\\' as u16,
				b'U' as u16, b'N' as u16, b'C' as u16, b'\\' as u16,
			];
			let mut wide: Vec<u16> = if buf.starts_with(UNC) {
				// `\\?\UNC\server\share` -> `\\server\share`
				let mut v = vec![b'\\' as u16, b'\\' as u16];
				v.extend_from_slice(&buf[UNC.len()..]);
				v
			} else if buf.starts_with(NS)
				&& buf.len() >= NS.len() + 2
				&& buf[NS.len() + 1] == b':' as u16
			{
				// `\\?\C:\path` -> `C:\path` (only when it's a real drive path)
				buf[NS.len()..].to_vec()
			} else {
				buf.to_vec()
			};
